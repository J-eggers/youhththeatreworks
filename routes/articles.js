...
router.get('/addArticle', ensureAuthenticated,adminAuthentication, (req, res)=>{
    res.render('articles/addArticle');
});
// Edit Article Form (admin only)
router.get('/editArticle/:id',ensureAuthenticated,adminAuthentication, (req, res)=>{
    Article.findOne({
        _id: req.params.id
    })
    .then(article =>{
