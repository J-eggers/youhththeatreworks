...
router.put('/register/:id',ensureAuthenticated, (req, res)=>{
    ...
    if(req.body.kidID){
        ...
        User.updateOne(idInput,{"kidname.$.registered" : true}, {new: true}, (err, user)=>{
            if (err) return (err);
            console.log('updated');
        });
...
router.delete('/register/:id',ensureAuthenticated, (req, res)=>{
    ...
    if(kidID){
        ...
        idInput = {_id:id, "kidname.name":kidname};
        User.updateOne(idInput,{"kidname.$.registered" : false}, {new: true}, (err, user)=>{
            if (err) return (err);
    ...
    Show.updateOne(idInput2, { $pull: { cast: {kidname : kidname, kidID: kidID } } }, (err,user)=>{
        if (err) return (err);
    })
    //redierect to show     
    Show.findOne({
...
router.post('/',ensureAuthenticated,adminAuthentication, (req, res)=>{
    ...
    User.updateMany({"kidname.registered": true},{$set:{"kidname.$[].registered" : false}},{multi:true}, (err, user)=>{
        if (err) return (err);
    ...
    if(errors.length > 0 ){
        ...
        new Show(userAdmin)
            .save()
            .then(show=> {
                User.updateMany({rank:"admin"}, {subscribed: show._id}, {new:true}, (err,user)=>{
                    if (err) return (err);
